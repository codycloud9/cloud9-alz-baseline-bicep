# Synnov Group — Azure Landing Zone Baseline Deployment

This deployment provisions a minimal, repeatable Azure landing zone baseline for **Synnov Group** using Bicep. It is designed for MSP operations and audit requirements by creating a consistent foundation for identity access, networking topology, and governance guardrails.

## What this deploys

### 1) Resource Organization
- Creates a dedicated networking resource group:
  - `netRgName` (example: `rg-synnov-platform-net-prod`)
...
- `parameters.json`  
  Synnov customer-specific configuration (names, CIDRs, RBAC principal, allowed locations).
...
- subnet prefixes for hub and workload networks
...
- Logged into the correct tenant/subscription:
  - `az login`
  - `az account set --subscription <SYNNOV_SUBSCRIPTION_ID>`
...
1) Validate:
```powershell
az deployment sub validate --location westus2 `
  --template-file deployments/synnov/main.bicep `
  --parameters @deployments/synnov/parameters.json
